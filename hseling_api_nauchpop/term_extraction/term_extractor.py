...
# частотный словарь
df = pd.read_csv('hseling_api_nauchpop/term_extraction/filters/freqrnc2011.csv', sep='\t')
...
# символьные n-граммы
with open('hseling_api_nauchpop/term_extraction/filters/ngrams/bigrams.txt') as fl:
    bi = fl.read()

with open('hseling_api_nauchpop/term_extraction/filters/ngrams/trigrams.txt') as fl:
    tri = fl.read()

with open('hseling_api_nauchpop/term_extraction/filters/ngrams/tetragrams.txt') as fl:
    tetra = fl.read()
...
# части слова
with open('hseling_api_nauchpop/term_extraction/filters/word_parts/suf2.txt') as fl:
    s2 = fl.read()

with open('hseling_api_nauchpop/term_extraction/filters/word_parts/suf3.txt') as fl:
    s3 = fl.read()

with open('hseling_api_nauchpop/term_extraction/filters/word_parts/suf4.txt') as fl:
    s4 = fl.read()

with open('hseling_api_nauchpop/term_extraction/filters/word_parts/pref2.txt') as fl:
    p2 = fl.read()

with open('hseling_api_nauchpop/term_extraction/filters/word_parts/pref3.txt') as fl:
    p3 = fl.read()

with open('hseling_api_nauchpop/term_extraction/filters/word_parts/pref4.txt') as fl:
    p4 = fl.read()
...
# стоп-слова
with open('hseling_api_nauchpop/term_extraction/filters/stopwords/stopwords.txt') as fl:
    SW = fl.read()
stopwords = SW.split('\n')
...
def get_term_candidates(text):
    ...
    for term in term_extractor(text):
        list_of_terms.append(term.normalized)
...
def pref_scores(list_of_terms):  # проверяем начало слов
    ...
    for term in list_of_terms:
        if term[:-4] in pref4:
            scores1.append(1)
        elif term[-3:] in pref3:
            scores1.append(1)
        elif term[:-2] in pref2:
            scores1.append(1)
        else:
            scores1.append(0)
